...
Baseline 모델로 **Test 98.41%**를 먼저 달성한 뒤, 회고에서 발견한 **2가지 문제에 직접 처방하고 1가지 설계 페어링을 더한 개선 모델 (v2)**로 **Test 98.62%** 도달
...
> Train 99.85%는 그 자체로 충분히 좋은 수치이며 "문제"로 분류할 만한 신호는 아닙니다. 다만 다음 §3에서 보겠지만 처방 ②(Dropout 강화)가 effective capacity를 줄이는 부작용이 있어, 이를 보완하기 위한 **설계 페어링**으로 모델 폭은 함께 키웁니다.

---

## 3. 개선 방향 — 2가지 처방 + 1가지 설계 페어링

§2의 두 문제 신호에 **1:1로 대응하는 처방 2개**를 선택하고, 그중 **처방 ②의 부작용을 보완하는 설계 페어링 1개**를 함께 묶었습니다.
...
**은닉층 폭 2배 (512/256 → 1024/512)**

- **새 문제에 대한 처방이 아니라 처방 ②의 동반 조정**
- **주된 이유**: Dropout 0.3 → 0.4로 정규화를 강하게 걸면 forward마다 살아남는 뉴런 비율이 줄어들어 **effective capacity가 감소**. 이 손실을 폭 증가로 보상 — "정규화 강화 + capacity 증가"는 표준 페어링
- **부수효과**: baseline의 train 99.85%가 진짜 capacity 상한인지도 자연히 확인됨 (회고는 §8 참조)
...
- 이론적으로 He 우위가 명백 → "한 번 더 음성 결과를 확인하는 실험"보다는 **위 2개 처방 + 1 페어링의 효과 확인이 더 의미 있다고 판단**

위 세 변경(처방 2 + 페어링 1)을 한 번에 적용한 결과물을 **"개선 모델 (v2)"**라고 부릅니다.
...
## 8. 처방·페어링 회고 — 의도대로 작동했나

세 변경(처방 2 + 페어링 1)을 동시에 적용한 단일 실험이라 효과 분리(ablation)는 한계지만, 관찰 신호로부터 다음을 추정합니다.

**처방 ① lr step decay — 의도대로, 효과 가장 또렷 (사실상 단독 기여로 확인)**
- 의도: 후반 saturation 해소
...
**현재 한계**
- **세 변경(처방 2 + 페어링 1)을 동시에 적용해 효과 분리가 어렵다** — 부록 E에서 v2 phase 1 = baseline 최종 정확도로 lr decay의 기여를 큰 폭 분리해냈지만, 폭 2배 단독·Dropout 0.4 단독 효과까진 ablation 필요
...
- **+0.21%p 향상은 사실상 처방 ①(lr step decay)이 단독으로 만들어낸 결과로 보임** — §8 회고의 추정을 데이터가 강하게 뒷받침
