...
Baseline 모델로 **Test 98.41%**를 먼저 달성한 뒤, 회고에서 발견한 **2가지 문제에 직접 해결 방법하고 1가지 설계 페어링을 더한 개선 모델 (v2)**로 **Test 98.62%** 도달
...
> Train 99.85%는 그 자체로 충분히 좋은 수치이며 "문제"로 분류할 만한 신호는 아닙니다. 다만 다음 §3에서 보겠지만 해결 방법 ②(Dropout 강화)가 effective capacity를 줄이는 부작용이 있어, 이를 보완하기 위한 **설계 페어링**으로 모델 폭은 함께 키웁니다.

---

## 3. 개선 방향 — 2가지 해결 방법 + 1가지 설계 페어링

§2의 두 문제 신호에 **1:1로 대응하는 해결 방법 2개**를 선택하고, 그중 **해결 방법 ②의 부작용을 보완하는 설계 페어링 1개**를 함께 묶었습니다.

### 해결 방법 — 관측된 문제에 직접 대응

| Baseline 문제 | 해결 방법 | 이론적 근거 |
| -- | -- | -- |
...
### 설계 페어링 — 해결 방법 ②의 부작용 보완

**은닉층 폭 2배 (512/256 → 1024/512)**

- **새 문제에 대한 해결 방법이 아니라 해결 방법 ②의 동반 조정**
...
- 이론적으로 He 우위가 명백 → "한 번 더 음성 결과를 확인하는 실험"보다는 **위 2개 해결 방법 + 1 페어링의 효과 확인이 더 의미 있다고 판단**
